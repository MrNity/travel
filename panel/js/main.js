    ...
    methods: {
        ...
        drawMap() {
            ...
            map.on('load', function() {
                ...
                el.style.backgroundImage = `url(../../server/public/images/icons/marker.svg)`
...
//                })
                ...
                axios.get(`http://localhost:8080/points?lat=${lat}&lng=${lon}`).then(res => {
                    let data = res.data
                    let features = []
	...
	text-overflow: ellipsis; white-space: nowrap;">${el.description}</p><p><img src="../public/images/${el.files[0]}" alt="${el.title}" width="200" height="150" style="object-fit: cover;"></p>`,
                                'icon': el.icon,
                                'link': `/place/${el.id}`,
                    ...
                    geojson.features.forEach(function(marker) {
                        ...
                        el.style.backgroundImage = `url(../../server/public/images/icons/${marker.properties.icon})`
            ...
            map.on('click', function(e) {
                this.selectedLon = e.lngLat.wrap().lng
                this.selectedLat = e.lngLat.wrap().lat
                console.log({lng: this.selectedLon, lat: this.selectedLat})
            })
        }
    },
    mounted() {
